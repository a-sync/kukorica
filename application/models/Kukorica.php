...
class Kukorica extends CI_Model {
    ...
    public function get_torrents_by_ids($torrent_ids, $select = 'torrent_id')
    {
        ...
        if(is_array($torrent_ids) && count($torrent_ids) > 0) {
            $torrent_ids = array_filter($torrent_ids);

            if(count($torrent_ids) > 0) {
                $query = $this->db
                    ->select($select)
                    ->from('torrents')
                    ->where('site_id', $this->site_id)
                    ->where_in('torrent_id', $torrent_ids)
                    ->get();
    ...
    public function get_movies_by_ids($imdb_ids, $select = 'imdb_id')
    {
        ...
        if(is_array($imdb_ids) && count($imdb_ids) > 0) {
            $imdb_ids = array_filter($imdb_ids);

            if(count($imdb_ids) > 0) {
                $query = $this->db
                    ->select($select)
                    ->from('movies')
                    ->where_in('imdb_id', $imdb_ids)
                    ->get();
    ...
    public function get_locked_movies_without_trailer($limit = NULL)
    {
        $query = $this->db
